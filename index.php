<?
require($_SERVER["DOCUMENT_ROOT"] . "/bitrix/header.php");
$APPLICATION->SetTitle("Парсер");

CModule::IncludeModule("iblock");

$el = new CIBlockElement;
$ibProperties = [];

$ibList = CIBlockElement::getList(
   [],
   ['IBLOCK_ID' => 6],
   false,
   false,
   ['ID', 'NAME']
);

while ($data = $ibList->GetNextElement()) {
   $arFields = $data->GetFields();
   $key = str_replace(['»', '«', '(', ')'], '', $arFields['NAME']);
   $key = strtolower($key);
   $arKey = explode(' ', $key);
   $key = '';
   foreach ($arKey as $part) {
      if (strlen($part) > 2) {
         $key = $key . trim($part) . ' ';
      }
   }
   $key = trim($key);
   $ibProperties['OFFICE'][$key] = $arFields['ID'];
}

$properyReq = CIBlockPropertyEnum::GetList(
   ["SORT" => "ASC", "VALUE" => "ASC"],
   ['IBLOCK_ID' => $IBLOCK_ID]
);
while ($propertyRel = $properyReq->Fetch()) {
   $key = trim($propertyRel['VALUE']);
   $ibProperties[$propertyRel['PROPERTY_CODE']][$key] = $propertyRel['ID'];
}

$listProperties = CIBlockElement::GetList([], ['IBLOCK_ID' => $IBLOCK_ID], false, false, ['ID']);
while ($element = $listProperties->GetNext()) {
   CIBlockElement::Delete($element['ID']);
}

if (($file = fopen('vacancy.csv', 'r')) !== false) {
   $first = 0;
   while (($data = fgetcsv($file, 1000, ',')) !== false) {
      if ($first == 0) {
         $first++;
         continue;
      }

      $PROP = array();
      $PROP['ACTIVITY'] = $data[9];
      $PROP['FIELD'] = $data[11];
      $PROP['OFFICE'] = $data[1];
      $PROP['LOCATION'] = $data[2];
      $PROP['REQUIRE'] = $data[4];
      $PROP['DUTY'] = $data[5];
      $PROP['CONDITIONS'] = $data[6];
      $PROP['EMAIL'] = $data[12];
      $PROP['DATE'] = date('d.m.Y');
      $PROP['TYPE'] = $data[8];
      $PROP['SALARY_TYPE'] = '';
      $PROP['SALARY_VALUE'] = $data[7];
      $PROP['SCHEDULE'] = $data[10];

      foreach ($PROP as $key => &$value) {
         $value = str_replace('\n', '', trim($value));
         if (stripos($value, '•') !== false) {
            $value = explode('•', $value);
            array_splice($value, 0, 1);
            foreach ($value as &$str) {
               $str = trim($str);
            }
         } elseif ($ibProperties[$key]) {
            $arSimilar = [];
            foreach ($ibProperties[$key] as $propKey => $propVal) {
               if ($key == 'OFFICE') {
                  $value = strtolower($value);
                  if ($value == 'центральный офис') {
                     $value .= 'свеза ' . $data[2];
                  } elseif ($value == 'лесозаготовка') {
                     $value = 'свеза ресурс ' . $value;
                  } elseif ($value == 'свеза тюмень') {
                     $value = 'свеза тюмени';
                  }
                  $arSimilar[similar_text($value, $propKey)] = $propVal;
               }
               if (stripos($propKey, $value) !== false) {
                  $value = $propVal;
                  break;
               }

               if (similar_text($propKey, $value) > 50) {
                  $value = $propVal;
               }
            }
            if ($key == 'OFFICE' && !is_numeric($value)) {
               ksort($arSimilar);
               $value = array_pop($arSimilar);
            }
         }
      }
      if ($PROP['SALARY_VALUE'] == '-') {
         $PROP['SALARY_VALUE'] = '';
      } elseif ($PROP['SALARY_VALUE'] == 'по договоренности') {
         $PROP['SALARY_VALUE'] = '';
         $PROP['SALARY_TYPE'] = $ibProperties['SALARY_TYPE']['договорная'];
      } else {
         $arSalary = explode(' ', $PROP['SALARY_VALUE']);
         if ($arSalary[0] == 'от' || $arSalary[0] == 'до') {
            $PROP['SALARY_TYPE'] = $ibProperties['SALARY_TYPE'][$arSalary[0]];
            array_splice($arSalary, 0, 1);
            $PROP['SALARY_VALUE'] = implode(' ', $arSalary);
         } else {
            $PROP['SALARY_TYPE'] = $ibProperties['SALARY_TYPE']['='];
         }
      }

      $arLoadProductArray = array(
         "MODIFIED_BY"    => $USER->GetID(),
         "IBLOCK_SECTION_ID" => false,
         "IBLOCK_ID"      => 6,
         "PROPERTY_VALUES" => $PROP,
         "NAME"           => $data[3],
         "ACTIVE"         => "Y",
      );

      if ($PRODUCT_ID = $el->Add($arLoadProductArray))
         echo "New ID: " . $PRODUCT_ID . "<br>";
      else
         echo "Error: " . $el->LAST_ERROR . "<br>";
   }
}

require($_SERVER["DOCUMENT_ROOT"] . "/bitrix/footer.php"); ?>